...
design_basic_gRNA = function(chr, start, end, genome) {
    ...
    # need to get this from chr, start, end
    # do it for both strands
    seqs =
...
getseq = function(dat,genome){
    ...
    rv = DNAStringSet(lapply(1:nrow(dat), function(i){
                subseq(genome[[levels(dat$chr)[dat$chr[i]]]],
                  start=as.numeric(levels(dat$start)[dat$start[i]]),
                  end=as.numeric(levels(dat$end)[dat$end[i]]))
            }))
